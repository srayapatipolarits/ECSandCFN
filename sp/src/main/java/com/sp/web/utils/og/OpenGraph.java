...
/**
 * A Java object representation of an Open Graph enabled webpage. A simplified layer over a
 * Hastable.
 *
 * @author Dax Abraham
 */
public class OpenGraph {
  ...
  private boolean isImported; // determine if the object is a new incarnation or representation of a
                              // web page
  ...
  static {
    ...
    BASE_TYPES.put("organization", new String[] { "band", "government", "non_profit", "school",
        "university" });
    BASE_TYPES.put("person", new String[] { "actor", "athlete", "author", "director", "musician",
        "politician", "profile", "public_figure" });
    BASE_TYPES.put("place", new String[] { "city", "country", "landmark", "state_province" });
    BASE_TYPES.put("product", new String[] { "album", "book", "drink", "food", "game", "movie",
   ...
   * Create an open graph representation for generating your own Open Graph object.
   */
  public OpenGraph(boolean isImported) {
    this();
   ...
   *          Set this option to true if you don't wish to have an exception throw if the page does
   *          not conform to the basic 4 attributes
   * @throws java.io.IOException
   *           If a network error occurs, the HTML parser will throw an IO Exception
   * @throws java.lang.Exception
   *           A generic exception is throw if the specific page fails to conform to the basic Open
   *           Graph standard as define by the constant REQUIRED_META
   */
  public OpenGraph(String url, boolean ignoreSpecErrors) throws java.io.IOException, Exception {
    this();
    isImported = true;
    
    // download the (X)HTML content, but only up to the closing head tag. We do not want to waste
    // resources parsing irrelevant content
...
//        break;
    ...
    for (TagNode metaElement : metaData) {
      for (OpenGraphNamespace namespace : pageNamespaces) {
        ...
        if (target != null
            && metaElement.getAttributeByName(target).startsWith(namespace.getPrefix() + ":")) {
  ...
  /**
   * Gets the charset for specified connection. Content Type header is parsed to get the charset
   * name.
   *
   * @param connection
   *          the connection.
   * @return the Charset object for response charset name; if it's not found then the default
   *         charset.
   */
  ...
  /**
   * Extract the charset name form the content type string. Content type string is received from
   * Content-Type header.
   *
   * @param contentType
  ...
  public String[] toHTML() {
    ...
    // return the array
    return returnHTML.toArray(NO_STRING);
  }
  ...
  /**
   * Test if the object has been modified by setters/deleters. This is only relevant if this object
   * initially represented a web page
   * 
  ...
  /**
   * @return
   *      - get the content reference object.
   */
  public ContentReference getContentRef() {
    ContentReference contentRef = null;
    if (isImported) {
      contentRef = new ContentReference();
      contentRef.setTitle(getContent("title"));
      contentRef.setDescription(getContent("description"));
      contentRef.setImageUrl(getContent("image"));
      contentRef.setAuthorSource(getContent("authorSource"));
      contentRef.setUrl(getContent("url"));
      SPMedia media = new SPMedia();
      media.setUrl(getContent("videoUrl"));
      if(getContent("type")!=null){
        switch (getContent("type")) {
        case "video":
          media.setMediaType(SPMediaType.Video);
          break;
        
        default:
          media.setMediaType(SPMediaType.Web);
          break;
        }  
      }
      
      contentRef.addMedia(media);
    }
    return contentRef;
  }
