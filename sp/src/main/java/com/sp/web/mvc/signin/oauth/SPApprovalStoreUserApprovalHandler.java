...
public class SPApprovalStoreUserApprovalHandler extends ApprovalStoreUserApprovalHandler {
  
  private static Logger logger = Logger.getLogger(SPApprovalStoreUserApprovalHandler.class);
   ...
   * 
   * @param scopePrefix
   *          the prefix (default {@link OAuth2Utils#SCOPE_PREFIX})
   */
  ...
  public AuthorizationRequest checkForPreApproval(AuthorizationRequest authorizationRequest,
    ...
    String userIdFromAuthentication = GenericUtils.getUserIdFromAuthentication(userAuthentication);
    ...
    if (clientDetailsService != null) {
      try {
        ...
        if (approvedScopes.containsAll(requestedScopes)) {
          // gh-877 - if all scopes are auto approved, approvals still need to be added to the
          // approval store.
          Set<Approval> approvals = new HashSet<Approval>();
          Date expiry = computeExpiry();
          for (String approvedScope : approvedScopes) {
            approvals
                .add(new Approval(userIdFromAuthentication, authorizationRequest.getClientId(),
        ...
        logger
            .warn("Client registration problem prevent autoapproval check for client=" + clientId);
      }
    }
    ...
    if (logger.isDebugEnabled()) {
      ...
      builder.append(" and username=" + userIdFromAuthentication);
      logger.debug(builder.toString());
    }
    
    // Find the stored approvals for that user and client
    Collection<Approval> userApprovals = approvalStore.getApprovals(userIdFromAuthentication,
        clientId);
  ...
  /**
   * Requires the authorization request to be explicitly approved, including all individual scopes,
   * and the user to be authenticated. A scope that was requested in the authorization request can
   * be approved by sending a request parameter <code>scope.&lt;scopename&gt;</code> equal to "true"
   * or "approved" (otherwise it will be assumed to have been denied). The {@link ApprovalStore}
  ...
  public AuthorizationRequest updateAfterApproval(AuthorizationRequest authorizationRequest,
    ...
    for (String requestedScope : requestedScopes) {
      ...
      if ("true".equals(value) || value.startsWith("approve")) {
        ...
        approvals.add(new Approval(userAuthentication.getName(),
            authorizationRequest.getClientId(), requestedScope, expiry, ApprovalStatus.APPROVED));
      } else {
        approvals.add(new Approval(userAuthentication.getName(),
            authorizationRequest.getClientId(), requestedScope, expiry, ApprovalStatus.DENIED));
      }
    }
    approvalStore.addApprovals(approvals);
  ...
  public Map<String, Object> getUserApprovalRequest(AuthorizationRequest authorizationRequest,
    ...
    for (Approval approval : approvalStore.getApprovals(userIdFromAuthentication,
