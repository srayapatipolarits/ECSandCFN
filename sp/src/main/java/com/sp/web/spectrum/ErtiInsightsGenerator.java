 ...
 */
@Component
public class ErtiInsightsGenerator {
  
  private static final Logger LOG = Logger.getLogger(ErtiInsightsGenerator.class);
  
  @Autowired
  private HiringGroupFactory groupFactory;
  /**
   * getErtiInsights method will give the ertiInsights for the user.
   * 
   * @param users
   *          list of the users
   * @return the Proifle Balance.
   */
  @Cacheable(value = "ertiInsights", key = "#companyId")
  public ErtiInsights getErtiInsights(String companyId, List<User> users, boolean paModule) {
    return getErtiInsight(companyId, users,  paModule);
    
  }
  
  private ErtiInsights getErtiInsight(String companyId, List<? extends User> users, boolean paModule) {
    ...
    List<ErtiUserInsights> specUsers = validUsers
        ...
        .map(
            usr -> {
              ...
              userDto.getErtiData().put(TraitType.Powerfull, powerFull);
              userDto.getErtiData().put(TraitType.Versatile, versatile);
              userDto.getErtiData().put(TraitType.Adaptable, adaptable);
              userDto.getErtiData().put(TraitType.Precise, precise);
              userDto.getErtiData().put(TraitType.BigPicture, (getAverage(powerFull, versatile)));
              userDto.getErtiData().put(TraitType.RelationOriented,
                  (getAverage(versatile, adaptable)));
              userDto.getErtiData().put(TraitType.DetailOriented, (getAverage(adaptable, precise)));
              userDto.getErtiData()
                  .put(TraitType.MissionOriented, (getAverage(powerFull, precise)));
    ...
    List<TraitType> traitsTypes = new ArrayList<TraitType>();
    traitsTypes.add(TraitType.Powerfull);
    traitsTypes.add(TraitType.Versatile);
    traitsTypes.add(TraitType.Adaptable);
    traitsTypes.add(TraitType.Precise);
    traitsTypes.add(TraitType.BigPicture);
    traitsTypes.add(TraitType.RelationOriented);
    traitsTypes.add(TraitType.DetailOriented);
    traitsTypes.add(TraitType.MissionOriented);
    
    traitsTypes.addAll(Arrays.asList(CategoryType.Processing.getTraitTypes()));
    traitsTypes.addAll(Arrays.asList(CategoryType.ConflictManagement.getTraitTypes()));
    traitsTypes.addAll(Arrays.asList(CategoryType.FundamentalNeeds.getTraitTypes()));
    traitsTypes.addAll(Arrays.asList(CategoryType.DecisionMaking.getTraitTypes()));
    spectrumFilter.getFilters().put(Constants.PARAM_TRAITS, traitsTypes);
    
    ErtiInsights ertiInsights = new ErtiInsights();
    ertiInsights.setSpectrumFilter(spectrumFilter);
    ertiInsights.setUserAnalytics(specUsers);
    ertiInsights.setCompanyId(companyId);
    return ertiInsights;
  }
  ...
  @CacheEvict(value = "ertiInsights", allEntries = true)
  public void reset() {
  ...
  public ErtiInsights getHiringErtiInsights(String companyId, List<HiringUser> validUsers, boolean paModule) {
    return getErtiInsight(companyId, validUsers, paModule);
  }
